...
<h3 id="steering-the-investigation">Steering the investigation</h3>

<p>An engineer can challenge the leading cause, ask Ada to check another signal, or add incident context from the Slack thread or workspace. Every steering request is scoped to one case and its current session. Before Ada runs, the harness records the operator's message and snapshots the current diagnosis version, its cited evidence, and the mitigation state. The message changes the next question Ada tests. It carries no authority to alter case state or approve an action.</p>

<p>The harness wraps the message as a follow&#8209;up instruction and resumes the existing Agent SDK session with <code>ClaudeAgentOptions(resume=session_id)</code>. The session brings back its earlier messages and tool results. The case file continues to hold confirmed findings, open hypotheses, and action state outside the transcript. The follow&#8209;up can end with a direct answer, which leaves the checked assessment and action state unchanged. Any change requires a new structured diagnosis with a revision reason. Diagnosis records are immutable, so the new record is appended and the earlier version stays in the case history.</p>

<p>New MCP tool results return to that same loop. Ada compares their timestamps and values with the working hypothesis and can change it before calling <code>record_diagnosis</code>. After a diagnosis has posted, every correction creates another version and passes through the original checks. The case service verifies that every citation names evidence captured for this case. The corroboration grader then checks that each cited excerpt supports its claim. A passing record becomes the current assessment. An unsupported claim is sent back into the same session with the failing claim named. A second failed pass leaves the record flagged for the engineer. Operator steering and grader feedback use the same resume path.</p>

<p>The workspace shows the direction under review, then reports whether the cause was confirmed or changed, which sources and findings were added, and whether the mitigation was refreshed. Mitigations are bound to a diagnosis version. A newer diagnosis makes a proposal tied to the older version stale at the gateway. If a still&#8209;unapproved mitigation fits, Ada resubmits it against the new record; the gateway clears its dry run and requires the checks and approval again. If the earlier proposal was already approved, the gateway's execution check invalidates that approval and clears the dry run before any change occurs. A changed cause leaves the earlier proposal unable to proceed.</p>

<p>In this example, the operator asks whether Kafka lag caused the failure. The resumed session checks lag onset, consumer throughput, and deployment history. Three additional sources and two findings support the original conclusion: the canary remains the cause and Kafka lag is downstream. The incident workspace reports that the assessment was confirmed, shows the new evidence, and marks the mitigation as updated.</p>
